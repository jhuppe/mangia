...
class SearchAssist extends Component {
  ...
  renderSuggestion(suggestion, { query, isHighlighted }) {
    ...
    return (
      <div>
        {parts.map((part, i) => {
          const className =
            part.toLowerCase() === query.toLowerCase() ? "highlight" : null;
  ...
  onSuggestionHighlighted({ suggestion }) {
    if (suggestion) this.props.onChange(suggestion.recipe);
  }
  ...
  onSuggestionsFetchRequested({ value }) {
    ...
    const suggestions = this.props.suggestions.filter(suggestion => {
      ...
      if (suggestion.category !== undefined)
        category =
          suggestion.category.slice(0, inputValue.length) === inputValue;
